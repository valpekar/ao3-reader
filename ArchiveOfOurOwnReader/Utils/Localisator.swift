...
//
//  Created by Valeriya Pekar on 11/20/18.
...
class Localisator {
    ...
    private var availableLanguagesArray         = ["DeviceLanguage", "English_en"]
    ...
    fileprivate func loadDictionaryForLanguage(_ newLanguage:String) -> Bool {
        ...
        for ext in arrayExt {
            if let path = Bundle(for: type(of: self)).url(forResource: "Localizable", withExtension: "strings", subdirectory: nil, localization: ext)?.path {
